	...
	function Genetic() {
		...
		this.start = async function() {
			...
			function mutateOrNot(entity) {
				...
				return Math.random() <= self.configuration.mutation && self.mutate ? self.mutate(Clone(entity)) : Clone(entity);
			}
			...
			for (i=0;i<this.configuration.iterations;++i) {
				...
				if (this.beforeFitness) {
				  await this.beforeFitness(this.entities);
        }
